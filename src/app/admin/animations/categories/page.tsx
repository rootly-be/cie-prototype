...
/**
 * Animation Categories Management Page
 * CRUD for animation categories
 */
export default function AnimationCategoriesPage() {
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch('/api/admin/categories?type=animation')
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/categories', {
        ...
        body: JSON.stringify({ nom: newName.trim(), type: 'animation' }),
      })
  ...
  return (
    <div>
      <div className={styles.pageHeader}>
        <h1 className={styles.pageTitle}>Catégories d&apos;animations</h1>
            ...
            {categories.length === 0 ? (
              ...
              categories.map((category) => (
                <tr key={category.id}>
                  ...
                  <td>{category._count?.animations || 0}</td>
