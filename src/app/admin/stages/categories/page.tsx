...
/**
 * Stage Categories Management Page
 * CRUD for stage categories
 */
export default function StageCategoriesPage() {
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch('/api/admin/categories?type=stage')
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/categories', {
        ...
        body: JSON.stringify({ nom: newName.trim(), type: 'stage' }),
      })
  ...
  return (
    <div>
      <div className={styles.pageHeader}>
        <h1 className={styles.pageTitle}>Catégories de stages</h1>
        <Link href="/admin/stages">
          <Button variant="outline">Retour aux stages</Button>
        </Link>
      </div>
            ...
            {categories.length === 0 ? (
              ...
              categories.map((category) => (
                <tr key={category.id}>
                  ...
                  <td>{category._count?.stages || 0}</td>
